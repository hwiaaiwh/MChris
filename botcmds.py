...
# I refuse to use AI in my code. Call it personal preference.
# Know that it cost me my sanity.
...
class querycmd(commands.Cog, name='query commands'):
    ...
    async def stopquery(self, ctx):
        ...
        if ch.id not in self.queries.keys(): 
            await ctx.send('jit it\'s not running')
        else:
                ...
                if self.queries[id][1]:
                    for player in curr:
                        if player not in self.queries[id][0]:
                            await ch.send(f'guys {player} stopped gooning (joined the server)')
                    for player in self.queries[id][0]:
                        if player not in curr:
                            await ch.send(f'guys {player} is definitely gooning (left the server)')
